...
function OcenaWydatkuWomenKcal({ sumaWydatkuMin, sumaWydatkuMax }) {
    ...
    return (
        <div className="w-full flex  h-full relative  items-center flex-col">
            ...
            <div className={`outline-none absolute -bottom-2 -right-3 md:bottom-2  md:right-2 w-16 h-16   rounded-full flex justify-center items-center hover:bg-niebieski-10 bg-zielony-1 text-niebieski-6  hover:text-white hover:border    cursor-pointer tracking-wider transition duration-300  `}  >
                <Link   href={
                    {
                        pathname: '/wydruk',
                        query: { pdf: JSON.stringify("wydatek") }
